...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (
      !targetUsername &&
      !['userGroup', 'batchUpdateUserGroups', 'setAllowRegister'].includes(
        action
      )
    ) {
    ...
    let configSavedAtomically = false;
    ...
    switch (action) {
      ...
      case 'add': {
        ...
        const savedConfig = await db.mutateUserAtomically({
          action: 'add',
          username: targetUsername!,
          password: targetPassword,
          config: adminConfig,
        });
        await setCachedConfig(savedConfig);
        configSavedAtomically = true;
        break;
      }
      case 'ban': {
        if (!targetEntry) {
