...
services:
  moontv-core:
    # 默认使用稳定版；如需开发版，请改为 ghcr.io/softmoonlit/lunatv:dev
    image: ghcr.io/softmoonlit/lunatv:latest
...
2. **添加 LunaTV 服务**
   ...
   - 点击 "Add Service" > "Docker Images"
   - 输入镜像名称：`ghcr.io/softmoonlit/lunatv:latest`
   - 如需测试开发版本，可改用：`ghcr.io/softmoonlit/lunatv:dev`
   - 配置端口：`3000` (HTTP)
...
> 💡 **提示**：
>
> - `latest` 是稳定版；如需测试开发版本，可将镜像标签改为 `dev`
> - 需要固定部署版本时，推荐使用短 SHA 标签（如 `sha-c954f96`），避免意外更新
